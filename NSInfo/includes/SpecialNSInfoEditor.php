 ...
 * @ingroup SpecialPage
 */
class SpecialNSInfoEditor extends SpecialPage
{
	/**
	...
	public function execute($par)
	{
		...
		$out->addModuleStyles('ext.nsinfo.specialpage');
	...
	public function onSubmit(array $data)
	{
		$status = Status::newGood();
		$request = $this->getRequest();
		$index = $this->getRequest()->getVal('index', '');
		...
		// (Bug 30599).
		$validPrefixChars = Title::legalChars();
		if ($do === 'add' && preg_match("/\s|[^$validPrefixChars]/", $pageName)) {
			$status->fatal('interwiki-badprefix', htmlspecialchars($pageName));
			return $status;
		}
		...
		$sql = NSInfoSql::getInstance();
		switch ($do) {
			case 'delete':
				return $sql->deleteNamespace($index, $pageName);
			case 'add':
				$nsInfo = NSInfoNamespace::fromNamespace($index, $pageName);
				// Fall through
			case 'edit':
	...
	protected function makeTable($canModify, $iwPrefixes)
	{
		...
		foreach ($iwPrefixes as $iwPrefix) {
			...
			$out .= Html::element(
				'td',
				['class' => 'mw-interwikitable-prefix'],
				$iwPrefix['iw_prefix']
			);
			$out .= Html::element(
				'td',
			...
			if ($canModify) {
				$out .= Html::rawElement(
					'td',
					['class' => 'mw-interwikitable-modify'],
